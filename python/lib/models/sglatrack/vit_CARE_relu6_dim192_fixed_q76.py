...
本檔為 vit_CARE_relu6 dim192 之 Q7.6 變體（檔名 ``vit_CARE_relu6_dim192_fixed_q76.py``）：
embed_dim=192 / num_heads=3；全路徑 ``to_fixed_point(..., 7, 6)``。
與 ``vit_CARE_relu6_fixed_q76.py``（768 維）同結構，factory 與預訓練載入對齊 dim192 student。
"""
import math
import logging
...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        q = to_fixed_point(q, 7, 6)
        k = to_fixed_point(k, 7, 6)
        v = to_fixed_point(v, 7, 6)
        ...
        qs = q * s
        ks = k * s
        qs = to_fixed_point(qs, 7, 6)
        ks = to_fixed_point(ks, 7, 6)
        q = F.relu6(qs)
        k = F.relu6(ks)
        q = to_fixed_point(q, 7, 6)
        k = to_fixed_point(k, 7, 6)
        ...
        k_mean = to_fixed_point(k_mean, 7, 6)
        z = 1.0 / (q @ k_mean.transpose(-2, -1) + 1e-5)
        z = to_fixed_point(z, 7, 6)
        kv = (k.transpose(-2, -1) @ v) / float(N)
        kv = to_fixed_point(kv, 7, 6)
        x = (q @ kv) * z
        x = to_fixed_point(x, 7, 6)
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        if return_attention:
            ...
            x_norm1 = to_fixed_point(x_norm1, 7, 6)
            feat, attn = self.attn(x_norm1, True)
            feat = to_fixed_point(feat, 7, 6)
            x = x + self.drop_path(feat)
            x = to_fixed_point(x, 7, 6)
            x_norm2 = self.norm2(x)
            x_norm2 = to_fixed_point(x_norm2, 7, 6)
            mlp_out = self.mlp(x_norm2)
            mlp_out = to_fixed_point(mlp_out, 7, 6)
            ...
            attn_out = to_fixed_point(attn_out, 7, 6)
...
def _create_vision_transformer_dim192_fixed_q76(pretrained=False, **kwargs):
    ...
    if pretrained:
        if isinstance(pretrained, str) and pretrained.endswith(".pth"):
            ...
            raise ValueError(
                "dim192 fixed Q7.6：pretrained 請為 ViT-Tiny .pth 或 sglatrack checkpoint 路徑"
            )
...
def vit_tiny192_care_fixed_q76_patch16_224(pretrained=False, **kwargs):
    """
    CARE ReLU6 Q7.6，embed_dim=192、depth=12、num_heads=3。
    state_dict 與浮點 ``vit_CARE_relu6_dim192`` 相容，供 test 時插入 Q7.6 截斷。
    """
    return _create_vision_transformer_dim192_fixed_q76(pretrained=pretrained, **kwargs)
