...
本檔為 vit_CARE_relu6 dim32 之 Q5.8 變體（檔名 ``vit_CARE_relu6_dim32_fixed_q58.py``）：
embed_dim=32 / num_heads=4；全路徑 ``to_fixed_point(..., 5, 8)``。
與 ``vit_CARE_relu6_dim32`` 同結構／state_dict 鍵名，供 test 時插入 Q5.8 截斷。
"""
import math
import logging
...
def _create_vision_transformer_dim32_fixed_q58(pretrained=False, **kwargs):
    ...
    if pretrained:
        if isinstance(pretrained, str) and (pretrained.endswith(".pth") or pretrained.endswith(".pth.tar")):
            ...
            raise ValueError(
                "dim32 fixed Q5.8：pretrained 請為 .pth / .pth.tar 或 sglatrack checkpoint 路徑"
            )

    return model


def vit_tiny32_care_fixed_q58_patch16_224(pretrained=False, **kwargs):
    """
    CARE ReLU6 Q5.8，embed_dim=32、depth=12、num_heads=4。
    state_dict 與浮點 ``vit_CARE_relu6_dim32`` 相容，供 test 時插入 Q5.8 截斷。
    """
    return _create_vision_transformer_dim32_fixed_q58(pretrained=pretrained, **kwargs)
