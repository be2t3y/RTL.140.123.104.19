...
本檔為 vit_CARE_relu6_fixed 之 Q7.7 變體（檔名 ``vit_CARE_relu6_fixed_q77.py``）：全路徑 ``to_fixed_point(..., 7, 7)``。
"""
import math
import logging
...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        q = to_fixed_point(q, 7, 7)
        k = to_fixed_point(k, 7, 7)
        v = to_fixed_point(v, 7, 7)
        ...
        qs = q * s
        ks = k * s
        qs = to_fixed_point(qs, 7, 7)
        ks = to_fixed_point(ks, 7, 7)
        q = F.relu6(qs)
        k = F.relu6(ks)
        q = to_fixed_point(q, 7, 7)
        k = to_fixed_point(k, 7, 7)
        ...
        k_mean = to_fixed_point(k_mean, 7, 7)
        z = 1.0 / (q @ k_mean.transpose(-2, -1) + 1e-5)
        z = to_fixed_point(z, 7, 7)
        kv = (k.transpose(-2, -1) @ v) / float(N)
        kv = to_fixed_point(kv, 7, 7)
        x = (q @ kv) * z
        x = to_fixed_point(x, 7, 7)
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        if return_attention:
            ...
            x_norm1 = to_fixed_point(x_norm1, 7, 7)
            feat, attn = self.attn(x_norm1, True)
            feat = to_fixed_point(feat, 7, 7)
            x = x + self.drop_path(feat)
            x = to_fixed_point(x, 7, 7)
            x_norm2 = self.norm2(x)
            x_norm2 = to_fixed_point(x_norm2, 7, 7)
            mlp_out = self.mlp(x_norm2)
            mlp_out = to_fixed_point(mlp_out, 7, 7)
            ...
            attn_out = to_fixed_point(attn_out, 7, 7)
...
def vit_base_patch16_224(pretrained=False, **kwargs):
    """
    ViT-Base (ViT-B/16)，與 vit.py 相同結構與權重鍵名；Attention 為 CARE ReLU6 softmax-free 前向，並含 Q7.7（7,7）截斷。
    """
    model_kwargs = dict(
