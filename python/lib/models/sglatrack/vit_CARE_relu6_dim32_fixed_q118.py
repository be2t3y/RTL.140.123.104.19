...
本檔為 vit_CARE_relu6 dim32 之 Q11.8 變體（檔名 ``vit_CARE_relu6_dim32_fixed_q118.py``）：
embed_dim=32 / num_heads=4；全路徑 ``to_fixed_point(..., 11, 8)``。
與 ``vit_CARE_relu6_dim32`` 同結構／state_dict 鍵名，供 test 時插入 Q11.8 截斷。
"""
import math
import logging
...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        q = to_fixed_point(q, 11, 8)
        k = to_fixed_point(k, 11, 8)
        v = to_fixed_point(v, 11, 8)
        ...
        qs = q * s
        ks = k * s
        qs = to_fixed_point(qs, 11, 8)
        ks = to_fixed_point(ks, 11, 8)
        q = F.relu6(qs)
        k = F.relu6(ks)
        q = to_fixed_point(q, 11, 8)
        k = to_fixed_point(k, 11, 8)
        ...
        k_mean = to_fixed_point(k_mean, 11, 8)
        z = 1.0 / (q @ k_mean.transpose(-2, -1) + 1e-5)
        z = to_fixed_point(z, 11, 8)
        kv = (k.transpose(-2, -1) @ v) / float(N)
        kv = to_fixed_point(kv, 11, 8)
        x = (q @ kv) * z
        x = to_fixed_point(x, 11, 8)
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        if return_attention:
            ...
            x_norm1 = to_fixed_point(x_norm1, 11, 8)
            feat, attn = self.attn(x_norm1, True)
            feat = to_fixed_point(feat, 11, 8)
            x = x + self.drop_path(feat)
            x = to_fixed_point(x, 11, 8)
            x_norm2 = self.norm2(x)
            x_norm2 = to_fixed_point(x_norm2, 11, 8)
            mlp_out = self.mlp(x_norm2)
            mlp_out = to_fixed_point(mlp_out, 11, 8)
            ...
            attn_out = to_fixed_point(attn_out, 11, 8)
...
def _create_vision_transformer_dim32_fixed_q118(pretrained=False, **kwargs):
    ...
    if pretrained:
        if isinstance(pretrained, str) and (pretrained.endswith(".pth") or pretrained.endswith(".pth.tar")):
            ...
            raise ValueError(
                "dim32 fixed Q11.8：pretrained 請為 .pth / .pth.tar 或 sglatrack checkpoint 路徑"
            )

    return model


def vit_tiny32_care_fixed_q118_patch16_224(pretrained=False, **kwargs):
    """
    CARE ReLU6 Q11.8，embed_dim=32、depth=12、num_heads=4。
    state_dict 與浮點 ``vit_CARE_relu6_dim32`` 相容，供 test 時插入 Q11.8 截斷。
    """
    return _create_vision_transformer_dim32_fixed_q118(pretrained=pretrained, **kwargs)
