...
本檔為 vit_CARE_relu6 dim192 之 Q10.8 變體（檔名 ``vit_CARE_relu6_dim192_fixed_q108.py``）：
embed_dim=192 / num_heads=3；全路徑 ``to_fixed_point(..., 10, 8)``。
與 ``vit_CARE_relu6_fixed_q108.py``（768 維）同結構，factory 與預訓練載入對齊 dim192 student。
"""
import math
import logging
...
def _create_vision_transformer_dim192_fixed_q108(pretrained=False, **kwargs):
    ...
    if pretrained:
        if isinstance(pretrained, str) and pretrained.endswith(".pth"):
            ...
            raise ValueError(
                "dim192 fixed Q10.8：pretrained 請為 ViT-Tiny .pth 或 sglatrack checkpoint 路徑"
            )
...
def vit_tiny192_care_fixed_q108_patch16_224(pretrained=False, **kwargs):
    """
    CARE ReLU6 Q10.8，embed_dim=192、depth=12、num_heads=3。
    state_dict 與浮點 ``vit_CARE_relu6_dim192`` 相容，供 test 時插入 Q10.8 截斷。
    """
    return _create_vision_transformer_dim192_fixed_q108(pretrained=pretrained, **kwargs)
